...
这个对拍机能够帮助您验证使用 Verilog 语言搭建的简易 MIPS 微系统是否能够正常工作。
该评测机会尽可能多地测试不同的中断、异常情况，但对指令功能是否正确实现并不会有太强的检测。指令功能的检测使用 P5 / P6 的对拍机即可。
...
1. **为防止不当修改 `$31` 的值，`handler` 程序中跳转仅用 `j` 指令实现，在评测前请添加这一指令的数据通路。**

2. 课程组提供的 MARS 可能存在输出“写入 0 号寄存器”信息的问题，比较评测结果时请留意。
...
4. `exception_handler` 目录下是异常处理代码，请不要删除目录下的文件，否则会导致评测结果无效。
