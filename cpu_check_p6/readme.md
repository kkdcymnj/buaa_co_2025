...
这个对拍机能够帮助您验证使用 Verilog 语言搭建的简易流水线 CPU 是否能够正常工作。
目前这个机器能够生成包含 P6 要求的所有指令的测试数据。
...
4. 在 `sample_manual` 目录下有一份特别构建的测试样例，用于提高转发/阻塞情况覆盖率，请视个人需求使用。
...
6. 输出比对：将对寄存器和对内存的操作分开比对，比对结果会在终端显示。
...
目前还有以下情况在测试数据中未能覆盖，希望后来人可以解决这一残留问题。
