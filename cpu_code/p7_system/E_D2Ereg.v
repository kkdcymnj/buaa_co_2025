...
module E_D2Ereg (
	...
	input [2:0] D_RegWriteDataSrc,
	input [31:0] D_calcTime,
	input [3:0] D_MultOp,
	...
	output reg [2:0] E_RegWriteDataSrc,
	output reg [31:0] E_calcTime,
	output reg [3:0] E_MultOp,
	...
	input D_cp0WriteEnable,
	input D_Syscall,
	input [4:0] D_cp0Addr,
	output reg E_handleException,
	output reg E_cp0WriteEnable,
	output reg E_Syscall,
	output reg [4:0] E_cp0Addr,

	input D_PCOutOfRange,
	input D_PCfalse,
	output reg E_PCOutOfRange,
	output reg E_PCfalse,

	input [4:0] D_exceptionCode,
	output reg [4:0] E_exceptionCodeTemp,

	input D_isLoadOrStore,
	output reg E_isLoadOrStore
);

wire stall = (D_stall);

always @(posedge clk) begin
	if (reset || stall || request) begin
		//阻塞时不要清空延迟槽、PC信息 
		E_rs <= 5'b0;
		E_rt <= 5'b0;
		E_RegWriteAddr <= 5'b0;
		...
		if(reset) begin
			...
			E_PC <= 32'h0000_4180;
			E_PCwith4 <= 32'h0000_4184;
			E_PCwith8 <= 32'h0000_4188;
		end
		else if(stall) begin
			E_PC <= D_PC;
			E_PCwith4 <= D_PCwith4;
		...
		E_isLoadOrStore <= 1'b0;
		E_exceptionCodeTemp <= 5'b1;	
		
		E_RI <= 1'b0;		  
		E_handleException <= 1'b0;
		E_cp0WriteEnable <= 1'b0;
		E_Syscall <= 1'b0;
		E_cp0Addr <= 5'b0;
		...
		E_BD <= D_BD;
		E_RI <= D_RI;
		E_handleException <= D_handleException;
		E_cp0WriteEnable <= D_cp0WriteEnable;
		E_Syscall <= D_Syscall;
		E_cp0Addr <= D_cp0Addr;

		E_PCOutOfRange <= D_PCOutOfRange;
		E_PCfalse <= D_PCfalse;

		E_exceptionCodeTemp <= D_exceptionCode;

		E_isLoadOrStore <= D_isLoadOrStore;
	end
end
